...
(async () => {
  ...
  await bet('r1', [3, 0]); await bet('rh', [3, 0]); await bet('r2', [0]); await bet('r3', [0, 3]);
  await bet('r4', [3, 3, 1]); await bet('r5', [2, 1]); await bet('r6', [1]);
  ...
  const entries = await page.$$eval('.entry-list li', els => els.length);
  console.log('codex:', codex, '| accumulated entries:', entries);
  if (entries < 5) errors.push('expected card ① (2) + card ⑦ (3) entries, got ' + entries);
  await shot('codex');
